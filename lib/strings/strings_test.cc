...
const str abcd = "abcd";
const str faces = "☺☻☹";
const str commas = "1,2,3,4";
const str dots = "1....2....3....4";
...
IndexTest last_string_any_tests[] = {
    {"", "", -1},
	{"", "a", -1},
	{"", "abc", -1},
	{"a", "", -1},
	...
	{"a.RegExp*", ".(|)*+?^$[]", 8},
	{dots + dots + dots, " ", -1},
	{"012abcba210", "\x{ff}b", 6},
	{"012\x{80}bcb\x{80}210", "\x{ff}b", 7},
	{"0123456\xcf\x{80}abc", "\x{cf}b\x80", 10},
} ;

void test_last_string_any(testing::T &t) {
    int i = 0;
    for (IndexTest const &tt : last_string_any_tests) {
        size actual = last_index_any(tt.s, tt.sep);
        if (actual != tt.out) {
            t.errorf("case %d: last_index_any(%q, %q) = %v; want %v", i, tt.s, tt.sep, actual, tt.out);
        }
        i++;
    }
}
...
IndexTest index_tests[] = {
	...
	{"abcd", "abcd", 0},
	{"xabcd", "abcd", 1},
	{str("xyabcd")[0,5], "abcd", -1},
	{"xbcqq", "abcqq", -1},
	{"abcqq", "abcqq", 0},
	{"xabcqq", "abcqq", 1},
	{str("xyabcqq")[0,6], "abcqq", -1},
	{"xabxcqq", "abcqq", -1},
	{"xabcqxq", "abcqq", -1},
	{"", "01234567", -1},
	{"32145678", "01234567", -1},
	...
	{str("xx01234567")[0,9], "01234567", -1},
	{"", "0123456789", -1},
	{"3214567844", "0123456789", -1},
	{"0123456789", "0123456789", 0},
	{"x0123456789", "0123456789", 1},
	{"x012345678x0123456789", "0123456789", 11},
	{str("xyz0123456789")[0,12], "0123456789", -1},
	...
	{str("xyz01234567890123456789")[0,22], "01234567890123456789", -1},
	{"", "0123456789012345678901234567890", -1},
	{"321456788901234567890123456789012345678911", "0123456789012345678901234567890", -1},
	{"0123456789012345678901234567890", "0123456789012345678901234567890", 0},
	{"x0123456789012345678901234567890", "0123456789012345678901234567890", 1},
	{"x012345678901234567890123456789x0123456789012345678901234567890", "0123456789012345678901234567890", 32},
	{str("xyz0123456789012345678901234567890")[0,33], "0123456789012345678901234567890", -1},
	{"", "01234567890123456789012345678901", -1},
	{"32145678890123456789012345678901234567890211", "01234567890123456789012345678901", -1},
	{"01234567890123456789012345678901", "01234567890123456789012345678901", 0},
	{"x01234567890123456789012345678901", "01234567890123456789012345678901", 1},
	{"x0123456789012345678901234567890x01234567890123456789012345678901", "01234567890123456789012345678901", 33},
	{str("xyz01234567890123456789012345678901")[0,34], "01234567890123456789012345678901", -1},
	{"xxxxxx012345678901234567890123456789012345678901234567890123456789012", "012345678901234567890123456789012345678901234567890123456789012", 6},
	{"", "0123456789012345678901234567890123456789", -1},
	{"xx012345678901234567890123456789012345678901234567890123456789012", "0123456789012345678901234567890123456789", 2},
	{str("xx012345678901234567890123456789012345678901234567890123456789012")[0,41], "0123456789012345678901234567890123456789", -1},
...
// in failure reports.
void run_index_tests(testing::T &t, std::function<size(str s, str sep)> f,  std::function<size(str s, str sep)> rf, str func_name, str func_name_rev, view<IndexTest> test_cases) {
    int i = 0;
	for (IndexTest const &test : test_cases) {
		size actual = f(test.s, test.sep);
		if (actual != test.out) {
			t.errorf("case %d: %s(%q, %q) = %v; want %v", i, func_name, test.s, test.sep, actual, test.out);
		}

		String s_rev = reverse(test.s);
		String sep_rev = reverse(test.sep);
		size out_rev_want = -1;
		if (test.out != -1) {
			out_rev_want = len(test.s) - test.out - len(test.sep);
		}
		size out_rev_got = rf(s_rev, sep_rev);
		if (out_rev_got != out_rev_want) {
			t.errorf("case %d reversed: %s(%q, %q) = %v; want %v", i, func_name_rev, s_rev, sep_rev, out_rev_got, out_rev_want);
		}
...
void test_index(testing::T &t)      { run_index_tests(t, (size(*)(str,str)) &strings::index, (size(*)(str,str)) &strings::last_index, "index", "last_index", index_tests); }
void test_last_index(testing::T &t) { run_index_tests(t, (size(*)(str,str)) &strings::last_index, (size(*)(str,str)) &strings::index, "last_index", "index", last_index_tests); }
	...
	for (int times = 0; times < 10; times++) {
		for (int str_len = 5 + rand_intn(5); str_len < 140; str_len += 10) { // Arbitrary
			...
			for (int i = 0; i < 50; i++) {
				...
				size res_rev = last_index(s_rev, sep_rev);
				if (res_rev != want_rev) {
...
const str benchmark_string = "some_text=some☺value";
...
void benchmark_index(testing::B &b) {
	if (size got = index(benchmark_string, "v"); got != 17) {
		b.fatalf("wrong index: expected 17, got=%d", got);
	}
	for (int i = 0; i < b.n; i++) {
		index(benchmark_string, "v");
	}
}
...
void test_split(testing::T &t) {
    auto to_vector = [](Split &&iter) {
        std::vector<str> out;
        for (str elem : iter) {
            out.push_back(elem);
        }
    ...
    int i = -1;
	for (SplitTest const &tt : splittests) {
        i++;
		std::vector<str> a = to_vector(split_n(tt.s, tt.sep, tt.n));
		if (a != tt.a) {
			t.errorf("case %d: split(%q, %q, %d) = %v (len %v); want %v (len %v)", 
                    i, tt.s, tt.sep, tt.n, a, a.size(), tt.a, tt.a.size());
			continue;
		}

        std::vector<str> b = split_n(tt.s, tt.sep, tt.n);
        if (b != tt.a) {
			t.errorf("case %db: split(%q, %q, %d) = %v (len %v); want %v (len %v)", 
                    i, tt.s, tt.sep, tt.n, b, b.size(), tt.a, tt.a.size());
			continue;
		}
		// if tt.n < 0 {
