...
const arr<UTF8Map> utf8map = (UTF8Map[]) {
    {0x0000, str("\x00")},
    {0x0001, "\x01"},
    {0x007e, "\x7e"},
    {0x007f, "\x7f"},
...
void test_full_rune(testing::T &t) {
    for (const UTF8Map & m : utf8map) {
        if (!full_rune(m.s)) {
            t.errorf("full_rule(%q) = false, want true", m.s);
        }
        
        str s1 = m.s[0, len(m.s)-1];
        if (full_rune(s1)) {
            t.errorf("full_rule(%q) = true, want false", m.s);
        }
    }
}

void test_encode_rune(testing::T &t) {
    for (const UTF8Map & m : utf8map) {
        Array<byte, 10> buf;
        str out = encode(buf, m.r);

        if (out != m.s) {
            t.errorf("encode(%v) = %q, want %q", m.r, out, m.s);
        }
    }
}

void test_encode_invalid_rune(testing::T &t) {
    rune r = 0x110000;
    str expected =  "\xef\xbf\xbd";
    Array<byte, 10> buf;

    if (str out = encode(buf, r); out != expected) {
        t.errorf("encode(0x%X) = %q, want %q", r, out, expected);
    }
}

void test_decode_rune(testing::T &t) {
    int i = 0;
    for (const UTF8Map & m : utf8map) {
        int runesize;
        rune r = utf8::decode_rune(m.s, runesize);
        if (r != m.r || runesize != len(m.s)) {
            t.errorf("decode(%q) = %v, runesize=%v; want %v, runesize=%v", m.s, runesize, r, m.r, len(m.s));
        }
        
        String s;
        s += m.s;
        s += '\0';
        r = utf8::decode_rune(s, runesize);
        if (r != m.r || runesize != len(m.s)) {
            t.errorf("decode");
        }
        ...
        r = utf8::decode_rune(m.s[0, len(m.s)-1], runesize);
        if (r != RuneError || runesize != wantsize) {
            t.errorf("decode");
        }
        ...
        // Runee Decoder
        {
            ...
            int bytes_consumed;
            bool ok, is_valid;
            rune r = decoder.decode_rune(m.s, true, bytes_consumed, ok, is_valid);
            if (r != m.r || bytes_consumed != len(m.s) || !ok || !is_valid) {
                t.errorf("case %d: RuneDecoder::decode(%q, eof=true) = %#U, ok=%v, is_valid=%v; want %#U", i, m.s, r, ok, is_valid, m.r);
            }
        }
        i++;
    }
...
struct {
    ...
    {"日本語", 1, "日"},
    {"日本語", 0, ""},
    {"\xE2\x98日本語", 2, "\xE2\x98"},
    {"\xE2\x98日本語", 3, "\xE2\x98日"},
    {"a\xe2\x80", 1, "a"},
    {"a\xe2\x80", 2, "a\xe2"},
    {"a\xe2\x80", 3, "a\xe2\x80"},
    {"\xF0\x9D\x8C", 3, "\xF0\x9D\x8C"},
};

void test_rune_truncate(testing::T &t) {
    for (size i = 0; auto const &test : rune_truncate_tests) {
        String out = rune_truncate(fmt::sprint(test.in), test.n);
        
        if (out != test.expected) {
            t.errorf("case %da: rune_truncate(%q, %v) = %q; want %q", i, test.in, test.n, out, test.expected);
        }


        struct ShortWriter : io::WriterTo {
            String s;
        ...
        ShortWriter sw;
        sw.s = test.in;
        out = rune_truncate(sw, test.n);

        if (out != test.expected) {
            t.errorf("case %db: rune_truncate(%q, %v) = %q; want %q", i, test.in, test.n, out, test.expected);
        }
...
void test_rune_count(testing::T &t) {
	for (size i = 0; RuneCountTest const &tt : runecounttests) {
		if (size out = rune_count(tt.in); out != tt.out) {
			t.errorf("case %va: rune_count(%q) = %d, want %d", i, tt.in, out, tt.out);
		}
		if (size out = rune_count(fmt::sprint(tt.in)); out != tt.out) {
			t.errorf("case %vb: rune_count(%q) = %d, want %d", i, tt.in, out, tt.out);
		}
        i++;
	}
}
