...
// void test_is_print(testing::T &t) {
// 	int n = 0;
// 	for (rune r = rune(0); r <= unicode::MaxRune; r++) {
// 		if (is_print(r) != unicode::is_print(r) {
// 			t.errorf("IsPrint(%U)=%t incorrect", r, is_print(r));
// 			n++;
// 			if (n > 10) {
// 				return;
// 			}
...
arr<QuoteTest> quotetests = (QuoteTest[]) {
	{"\a\b\f\r\n\t\v", R"("\a\b\f\r\n\t\v")", R"("\a\b\f\r\n\t\v")", R"("\a\b\f\r\n\t\v")"},
	{"\\", R"("\\")", R"("\\")", R"("\\")"},
	{"abc\x{ff}def", R"("abc\xffdef")", R"("abc\xffdef")", R"("abc\xffdef")"},
	{"\u263a", R"("☺")", R"("\u263a")", R"("☺")"},
	{"\U0010ffff", R"("\U0010ffff")", R"("\U0010ffff")", R"("\U0010ffff")"},
	{"\x04", R"("\x04")", R"("\x04")", R"("\x04")"},
	// Some non-printable but graphic runes. Final column is double-quoted.
	{"!\u00a0!\u2000!\u3000!", R"("!\u00a0!\u2000!\u3000!")", R"("!\u00a0!\u2000!\u3000!")", "\"!\u00a0!\u2000!\u3000!\""},
	{"\x7f", R"("\x7f")", R"("\x7f")", R"("\x7f")"},
};

void test_quote(testing::T &t) {
    int cnt = 0;
	for (QuoteTest &tt : quotetests) {
		if (String out = quote(tt.in); out != tt.out) {
			t.errorf("case %da: quote(%s) = %s, want %s", cnt, tt.in, out, tt.out);
		}

		if (String out = quote(fmt::sprint(tt.in)); out != tt.out) {
			t.errorf("case %db: quote(WriterTo(%s)) = %s, want %s", cnt, tt.in, out, tt.out);
		}
...
void test_quote_to_ascii(testing::T &t) {
	int cnt = 0;
	for (QuoteTest &tt : quotetests) {
		if (String out = quote_to_ascii(tt.in); out != tt.ascii) {
			t.errorf("case %da: quote_to_ascii(%s) = %s, want %s", cnt, tt.in, out, tt.ascii);
		}
		if (String out = quote_to_ascii(fmt::sprint(tt.in)); out != tt.ascii) {
			t.errorf("case %db: quote_to_ascii(WriterTo(%s)) = %s, want %s", cnt, tt.in, out, tt.ascii);
		}
		cnt++;
	}
}
