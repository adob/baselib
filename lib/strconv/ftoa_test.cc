...
const float64 below1e23 = 99999999999999974834176.0;
const float64 above1e23 = 100000000000000008388608.0;
...
const FloatTest floattests[] = {
	...
	// Issue 29491.
	{498484681984085570.0, 'f', -1, "498484681984085570"},
...
void test_float(testing::T &t) {
    int i = 0;
    for (FloatTest const &test : floattests) {
        String s = format_float(test.f, test.fmt, test.prec);
        if (s != test.s) {
            t.errorf("case %d: format_float(%v, %q, %v) = %v; want %v", i, test.f, test.fmt, test.prec, s, test.s);
        }
        i++;
    }
}