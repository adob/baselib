...
namespace {
    ...
    bool is_x(arr<Range> ranges, rune r) {
...
bool unicode::is_excluding_latin(RangeTable const& rangetab, rune r) {
    arr<Range16> r16 = rangetab.r16;
    int off = rangetab.latin_offset;
    if (len(r16) > off && r < r16[len(r16)-1].hi) {
        return is_x(r16(off), r);
    }
    arr<Range32> r32 = rangetab.r32;
